...
from oven_engine_3D.meshes import SphereMesh, CubeMesh
...
class Assignment5(BaseApp3D):
    def __init__(self):
        ...
        mat3 = MeshShader(diffuse_texture="res/textures/cubemap2.png", params = {"unshaded": True})
        self.objects.append(Cube(uv_mode=CubeMesh.UVMode.CROSS, parent_app=self, origin=Vector3D.LEFT*3. + Vector3D.FORWARD * 3., shader=mat3))
        ...
        self.objects.append(Sphere(parent_app=self, origin=Vector3D.BACKWARD * 10. + Vector3D.RIGHT * 5., shader=moon_mat, scale = .5))

        self.p = Vector3D.ZERO
